...
int alloc1(void) {
	...
	for (i = 0; year <= MAXY; mnth = mnth % 12 + 1, year += (mnth == 1), i++) {
		...
		if (((T + i)->hash = (htab ***)malloc(MNMC*sizeof(htab **))) == NULL)
			return 1;

		for (j = 0; j < MNMC; j++)
		if (((T + i)->hash[j] = (htab **)malloc(2 * sizeof(htab *))) == NULL)
			return 1;

		for (j = 0; j < MNMC; j++)
		if (((T + i)->hash[j][0] = (htab *)malloc(ECF * sizeof(htab))) == NULL)
			return 1;

		for (j = 0; j < MNMC; j++)
		if (((T + i)->hash[j][1] = (htab *)malloc(ECF * sizeof(htab))) == NULL)
			return 1;
...
int alloc2(int M_n) {
	...
	for (i = 0; i < S.N; i++)
		if ((S.cgrp[i].beta = (double *)malloc((M_n+1)*sizeof(ecf *))) == NULL)
			return 1;
